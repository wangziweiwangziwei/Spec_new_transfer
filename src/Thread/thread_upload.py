...
class SendFileThread(threading.Thread): 
    ...
    def run(self):
        while(1):
            ...
            #
            # if(not self.queueAbUpload._qsize()==self.queueFFTUpload._qsize()) :
            #     print self.queueAbUpload._qsize()
            #     print self.queueFFTUpload._qsize()
                # raise Exception('pao chu l e yige   excepthion ----------1！！！！！！！！！！！！！！！！')
            ...
            if(self.fileUploadMode==0 ):
                ...
                while((not self.queueAbUpload.empty()) and (not self.queueFFTUpload.empty())):
                    if(staticFileUp.getUploadMode()==2):
                        ...
                        break 
                    self.extract_num+=1
                    if(self.extract_num>=staticFileUp.getExtractM()): # only change extractM
